...
const SAMPLE_PASTE = `Day 1 - Arrive Amsterdam, Jordaan walk & canal-side dinner
Day 2 - Rijksmuseum, Vondelpark, Museum Quarter lunch
Day 3 - Anne Frank House, canal cruise, Nine Streets shopping
Day 4 - De Pijp food tour, Albert Cuyp Market, Heineken Experience
Day 5 - Day trip to Zaanse Schans windmills, cheese tasting, farewell dinner in Centrum`;
...
export default function IntakeFlow() {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.5, maxWidth: 300 }}>
          Turn any Amsterdam itinerary into an offline trip companion with local directions, budget tracking, and insider tips.
        </div>
      </div>
      ...
      <PathCard
        ...
        desc="Popular routes you can customize — Canal Ring, Museums, Day Trips & more"
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: COLORS.deepNight, lineHeight: 1.6, fontStyle: 'italic' }}>
          "We were lost in the Jordaan and the app had everything — directions, restaurant picks, even the right phrases. Felt like traveling with a local friend."
        </div>
        <div style={{ fontSize: 12, fontWeight: 700, color: COLORS.oceanTeal, marginTop: 8 }}>
          — Sarah & Mike, 5-day Amsterdam trip
        </div>
      </div>
    </>
  );
  ...
  const TemplatesScreen = () => {
    ...
    const filtered = filter === 'all'
      ? templates
      : templates.filter((t) => {
          if (filter === 'centrum') return t.route.match(/Centrum|Red Light District|Jordaan/i);
          if (filter === 'south') return t.route.match(/De Pijp|Museum Quarter|Vondelpark/i);
          return true;
        });
    ...
    <>
      ...
      {!apiError && (
        <>
          ...
          {[
            { icon: '🧭', title: 'Real Directions', desc: "Not Google Maps pins — actual 'take tram 2 from Centraal, exit at Rijksmuseum' directions" },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '💬', title: 'Local Phrases', desc: 'Dutch phrases tuned to your destinations with pronunciation' },
            ...
            { icon: '💵', title: 'Tipping Guide', desc: 'What to tip in every Amsterdam situation, from canal boat captains to restaurant servers' },
            { icon: '🌅', title: 'Sunrise/Sunset', desc: 'Golden hour times with best viewpoints matched to your daily locations' },
            { icon: '📶', title: 'Works Offline', desc: 'Everything cached to your phone. No signal underground? No problem' },
          ].map((f, i) => (
            <div key={i} style={{
