...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'canal': ['centrum', 'jordaan', 'plantage'],
    'museum': ['museum-quarter', 'centrum', 'plantage'],
    'art': ['museum-quarter', 'centrum', 'jordaan'],
    'culture': ['centrum', 'museum-quarter', 'jordaan'],
    'history': ['centrum', 'jordaan', 'plantage'],
    'food': ['de-pijp', 'jordaan', 'centrum'],
    'beer': ['jordaan', 'centrum', 'de-pijp'],
    'relax': ['vondelpark-area', 'jordaan', 'amsterdam-north'],
    'adventure': ['amsterdam-north', 'zaanse-schans', 'centrum'],
    'family': ['centrum', 'vondelpark-area', 'zaanse-schans'],
    'honeymoon': ['jordaan', 'centrum', 'museum-quarter'],
    'romantic': ['jordaan', 'centrum', 'museum-quarter'],
    'budget': ['de-pijp', 'amsterdam-north', 'vondelpark-area'],
    'luxury': ['centrum', 'museum-quarter', 'jordaan'],
    'shopping': ['centrum', 'jordaan', 'de-pijp'],
    'nightlife': ['centrum', 'de-pijp', 'red-light-district'],
    'day-trip': ['zaanse-schans', 'haarlem', 'keukenhof'],
    'tulip': ['keukenhof', 'centrum', 'vondelpark-area'],
    'windmill': ['zaanse-schans', 'centrum', 'amsterdam-north'],
  };
  ...
  return ['centrum', 'jordaan', 'museum-quarter'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Amsterdam AI Trip Planner. You generate detailed day-by-day itineraries for Amsterdam travel.
...
- Be specific: real place names, real prices, real transport options (GVB trams/metro, NS trains, canal boats, OV-chipkaart)
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-centrum"
- Transport between neighborhoods should reference GVB trams/metro, cycling, walking, or NS trains for day trips (e.g. Amsterdam Centraal to Haarlem)
...
- 15+ days: 2-3 items per day, brief descriptions (under 15 words each). Group similar days (e.g. "Days 5-6: Exploring Dutch island villages"). Only include key activities, one meal, and accommodation.
...
{
  "title": "string — trip title like '7-Day Northern Amsterdam Grand Tour'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "eur": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Amsterdam'",
      "destination": "string — slug like 'centrum'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Rijksmuseum', 'Anne Frank House', 'Vondelpark'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Amsterdam trip:\n${parts.join('\n')}`;
