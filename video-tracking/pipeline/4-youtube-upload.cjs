 ...
 *   2. Create project "Discover Amsterdam"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Amsterdam Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'amsterdam'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Amsterdam ${year}`,
      description: `${title}\n\nFull guide: https://discoveramsterdam.info/${slug}/`,
      tags: [`Amsterdam ${slug}`, 'Amsterdam travel', `Amsterdam ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Amsterdam ✈️ #shorts`,
      description: `${title} — Full guide: https://discoveramsterdam.info/destinations/${slug}/`,
      tags: [`${title} Amsterdam`, 'Amsterdam travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Amsterdam — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Amsterdam for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoveramsterdam.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoveramsterdam.info/companion/

#Amsterdam #${title.replace(/\s+/g, '')} #AmsterdamTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    ...
    'Amsterdam travel',
    'Amsterdam food',
    'Amsterdam budget travel',
    'Amsterdam tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Amsterdam`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER AMSTERDAM              ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
